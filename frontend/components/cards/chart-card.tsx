    ...
    <Card className="space-y-3 sm:space-y-4 p-3 sm:p-4 w-full">
      ...
      <div className="w-full overflow-hidden">
        <div className="relative h-48 sm:h-56 md:h-64 w-full">
          <svg className="w-full h-full" viewBox="0 0 1000 300" preserveAspectRatio="xMidYMid meet">
            ...
            {chartData.traces.map((trace: any, traceIdx: number) => {
              ...
              const points = trace.depths.map((depth: number, i: number): { x: number; y: number; value: number; depth: number; idx: number } => {
                const x = 60 + ((trace.values[i] - minValue) / (maxValue - minValue)) * 880;
                const y = 260 - ((depth - minDepth) / (maxDepth - minDepth)) * 240;
          ...
          {hovered && (() => {
            ...
            const xPercent = ((60 + ((value - minValue) / (maxValue - minValue)) * 880) / 1000) * 100;
            const yPercent = ((260 - ((depth - minDepth) / (maxDepth - minDepth)) * 240) / 300) * 100;
            return (
              <div
              ...
              >
                ...
                <div className="text-muted-foreground text-xs">Depth: {depth}m</div>
                <div className="text-muted-foreground text-xs">{chartData.variable}: {value.toFixed(2)} {chartData.units}</div>
              </div>
            );
          })()}
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4 text-xs text-muted-foreground bg-muted/20 rounded-lg p-2 sm:p-3">
        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4">
          <span className="whitespace-nowrap">Range: {minValue.toFixed(2)} - {maxValue.toFixed(2)} {chartData.units}</span>
